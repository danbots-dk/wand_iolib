...
static int cap11xx_i2c_probe(struct i2c_client *i2c_client, const struct i2c_device_id *id)
{
	...
	dev_info(dev, "Test CAP12XX detected, model %s, revision 0x%02x\n",
		 id->name, rev);
	node = dev->of_node;
	...
	if (id->driver_data  == CAP1293) {
		error = regmap_write(priv->regmap, 0x29, 0x01);  // Write 0x01 to register 29h
        if (error) {
            dev_err(dev, "Failed to configure Signal Guard\n");
            return error;
        }
        dev_info(dev, "Signal guard register configured with 0x01\n");
	}
